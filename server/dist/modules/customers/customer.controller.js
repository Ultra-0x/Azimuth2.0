...
const customer_schemas_js_1 = require("./customer.schemas.js");
...
async function getMyProfile(req, res) {
    ...
    try {
        const customer = await (0, customer_service_js_1.getCustomerProfile)(req.user.id);
        if (!customer) {
            res.status(404).json({
...
async function updateMyProfile(req, res) {
    ...
    const validation = customer_schemas_js_1.updateCustomerProfileSchema.safeParse(req.body);
    if (!validation.success) {
        ...
        const phoneIssue = issues.find((issue) => issue.path[0] === "phone");
        const unknownFieldIssue = issues.find((issue) => issue.code === "unrecognized_keys");
        if (firstNameIssue) {
            res.status(400).json({
        ...
        if (unknownFieldIssue) {
            res.status(400).json({
                ...
                message: "Only firstName, lastName, and phone can be updated.",
            });
            return;
        }
        res.status(400).json({
    ...
    try {
        const customer = await (0, customer_service_js_1.updateCustomerProfile)(req.user.id, updateData);
        res.status(200).json({
        ...
        if (error instanceof customer_service_js_1.CustomerError) {
